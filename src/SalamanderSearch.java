...
public class SalamanderSearch {
    ...
    public static List<int[]> possibleMoves(char[][] enclosure, int[] current)
    {
        ...
        newR = curR + 1;
        newC = curC;
        if (newR < enclosure.length && enclosure[newR][newC] != 'W') moves.add(new int[]{newR, newC});
        // Left
        newR = curR;
        newC = curC - 1;
        if (newC >= 0 && enclosure[newR][newC] != 'W') moves.add(new int[]{newR, newC});
        // Right
        newR = curR;
        newC = curC + 1;
        if (newC < enclosure[newR].length && enclosure[newR][newC] != 'W') moves.add(new int[]{newR, newC});
